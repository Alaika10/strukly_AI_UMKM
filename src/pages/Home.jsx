...
const Home = ({ transactions }) => {
  ...
  const [isTaxModalOpen, setIsTaxModalOpen] = useState(false); // Modal Pajak
  ...
  const estimasiPajak = omset * 0.005; // 0.5% dari Omset

  return (
    <div className="p-8 lg:p-12 relative">
      <header className="mb-10 max-w-5xl">
        <h1 className="text-4xl font-extrabold text-on-surface tracking-tight mb-2">Halo, Warung Berkah!</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 max-w-6xl">
        <StatCard title="Total Uang Masuk" amount="Rp 12.450.000" trend="+12%" type="inflow" />
        <StatCard title="Total Uang Keluar" amount="Rp 4.210.000" trend="-5%" type="outflow" />
        <StatCard title="Sisa Saldo" amount="Rp 8.240.000" type="primary" progress={65} />
      </div>
      ...
      <section className="mb-12 max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <ActionButton 
            ...
            onClick={() => setIsTaxModalOpen(true)} // Buka modal pajak
          />
        </div>
      </section>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 sm:p-6">
          ...
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl relative z-10 flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 lg:p-8 overflow-y-auto no-scrollbar flex-1 space-y-8">
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100/50">
                <div className="relative flex items-center">
                  ...
                  <input type="number" placeholder="0" className="w-full bg-transparent border-none py-2 pl-14 pr-4 text-5xl font-black text-slate-800 focus:ring-0 outline-none" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <div className="flex gap-3 h-[88px]">
                    <button onClick={() => setIncomeSource('langsung')} className={`flex-1 rounded-xl font-bold text-sm flex flex-col items-center justify-center gap-1.5 transition-all border-2 ${incomeSource === 'langsung' ? 'bg-[#003d9b] text-white border-transparent shadow-md' : 'bg-slate-50 text-slate-500 hover:bg-slate-100 border-transparent hover:border-slate-200'}`}>
                    ...
                    <button onClick={() => setIncomeSource('online')} className={`flex-1 rounded-xl font-bold text-sm flex flex-col items-center justify-center gap-1.5 transition-all border-2 ${incomeSource === 'online' ? 'bg-[#003d9b] text-white border-transparent shadow-md' : 'bg-slate-50 text-slate-500 hover:bg-slate-100 border-transparent hover:border-slate-200'}`}>
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative h-[88px]">
                    <select className="bg-none w-full h-full bg-slate-50 hover:bg-slate-100 border-none rounded-xl px-5 font-bold text-slate-700 text-base focus:ring-2 focus:ring-[#003d9b]/20 outline-none appearance-none cursor-pointer transition-colors">
                      <option>Makanan Berat</option><option>Minuman Sejuk</option><option>Camilan / Side Dish</option><option>Paket Katering</option>
                    </select>
            ...
            <div className="p-6 border-t border-slate-100 bg-white shrink-0">
              <button className="w-full py-4 bg-[#003d9b] text-white font-bold rounded-xl hover:bg-[#003d9b]/90 active:scale-[0.98] transition-all shadow-lg shadow-[#003d9b]/20 flex items-center justify-center gap-2">
          ...
          <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl relative z-10 flex flex-col max-h-[95vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 lg:p-10 overflow-y-auto no-scrollbar flex-1 space-y-10">
              <div className="flex flex-col items-center text-center">
                <div className="w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] p-10 flex flex-col items-center gap-6 group hover:border-[#003d9b]/50 transition-colors cursor-pointer">
                  <div className="w-20 h-20 bg-white rounded-2xl shadow-sm flex items-center justify-center">
                    <span className="material-symbols-outlined text-4xl text-slate-300">image</span>
                  </div>
                  <div>
                    <h4 className="text-xl font-bold text-slate-800 mb-2">Smart Scan AI</h4>
                    <p className="text-sm text-slate-400 max-w-xs mx-auto leading-relaxed">Tarik dan lepas struk belanja Anda di sini, atau klik untuk memilih file dari perangkat.</p>
                  </div>
                  <button className="bg-[#003d9b] text-white px-8 py-3 rounded-xl font-bold text-sm shadow-lg shadow-blue-900/20 active:scale-95 transition-all">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                <div className="space-y-2">
                  ...
                  <input type="text" placeholder="Contoh: Toko Beras Jaya" className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-semibold focus:ring-2 focus:ring-[#003d9b]/20 outline-none" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold tracking-widest text-slate-500 uppercase">Nominal (IDR)</label>
                  <input type="number" placeholder="0" className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-semibold focus:ring-2 focus:ring-[#003d9b]/20 outline-none" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold tracking-widest text-slate-500 uppercase">Kategori</label>
                  <div className="relative">
                    <select className="bg-none w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-semibold focus:ring-2 focus:ring-[#003d9b]/20 outline-none appearance-none cursor-pointer">
                      <option>Pilih Kategori</option><option>Bahan Baku</option><option>Listrik & Air</option><option>Gaji Karyawan</option>
                    </select>
                    <span className="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-400">keyboard_arrow_down</span>
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <input type="date" className="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-semibold focus:ring-2 focus:ring-[#003d9b]/20 outline-none" />
                </div>
              </div>
            </div>
            <div className="p-6 border-t border-slate-100 bg-white">
              <button className="w-full py-4 bg-[#003d9b] text-white font-bold rounded-xl hover:bg-[#003d9b]/90 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                Simpan Pengeluaran
              </button>
            </div>
          </div>
      ...
      {/* --- MODAL 3: HITUNG PAJAK (SESUAI GAMBAR BARU) --- */}
      {isTaxModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 sm:p-6">
          ...
          <div className="bg-white w-full max-w-xl rounded-3xl shadow-2xl relative z-10 flex flex-col max-h-[95vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 lg:p-8 overflow-y-auto no-scrollbar flex-1 space-y-6">
              ...
              <div className="bg-[#003d9b] p-6 lg:p-8 rounded-2xl text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 shadow-lg shadow-blue-900/20">
                ...
                <button className="bg-white text-[#003d9b] px-5 py-3.5 rounded-xl font-bold text-sm shadow-sm hover:bg-slate-50 active:scale-95 transition-all flex items-center gap-2 whitespace-nowrap">
